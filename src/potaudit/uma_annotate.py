...
def _build_uma_calculator(
    *,
    ...
    """
    Build UMA calculator exactly like your working script:
...
def annotate_extxyz_with_uma(
    *,
    ...
    for atoms in frames:
        ...
        f_uma = np.asarray(a.get_forces(), float) # eV/Å
        ...
        if add_deltas:
            # Compare to VASP (already stored in your extxyz)
            ...
            if f_vasp is not None:
                dv = f_uma - np.asarray(f_vasp, float)
    ...
    return UMAAnnotateReport(
        written_frames=len(annotated),
